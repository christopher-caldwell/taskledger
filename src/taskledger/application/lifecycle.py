...
class LifecycleOwner:
    ...
    def __init__(self, service, project, principal, run_id, config, runtime, *, owned_lease=None,
                 runner_factory=None, pause_result_factory=None, cleanup=None):
        ...
        self._pause_result_factory = pause_result_factory
    ...
    async def run(self):
        ...
        lease = self._owned_lease or self.journal.project_lock(self.project["id"])
        entered_here = self._owned_lease is None
        if entered_here:
            lease.__enter__()
        try:
            if self._runner_factory is None:
                # Resolve lazily so diagnostic callers can substitute a controller.
                from taskledger.controller.project import ProjectController
                execution = ProjectController(
                    service=self.service, project=self.project, orchestrator=self.principal,
                    run_id=self.run_id, runtime=self.runtime, journal=self.journal,
                    config=self.config, stop_requested=self._stop.is_set, owns_project_lease=False,
                ).run()
            else:
                execution = self._runner_factory(self._stop.is_set)
            controller = asyncio.create_task(execution)
            ...
            try:
                ...
                for turn in self.journal.open_turns(project_id=self.project["id"]):
                    ...
                    if turn.external_turn_id:
                        ...
                        with suppress(Exception):
                            await asyncio.wait_for(self.runtime.interrupt_turn(handle), timeout=5)
                        try:
                            inspection = None
                            for _ in range(10):
                                inspection = await asyncio.wait_for(self.runtime.inspect_turn(handle), timeout=1)
                                result = inspection.result
                                if inspection.state in {"FAILED", "COMPLETED"}:
                                    break
                                await asyncio.sleep(.1)
                            if inspection and inspection.state == "FAILED":
                                self.journal.fail_turn(turn.id, inspection.error or "INTERRUPTED", uncertain=False, result=result)
                            ...
                            pass
                    self.journal.fail_turn(turn.id, "interrupted before a terminal provider result was proven", uncertain=True, result=result)
                ...
                if self._pause_result_factory is not None:
                    return self._pause_result_factory(self._reason, self.journal.usage(run_id=self.run_id))
