...
Before creating that wave, run `project preflight` with the selected profiles and declared local inputs/services. Treat profile-file presence as configuration evidence only. The host must separately confirm that it can launch the exact named agent; preserve its reported model and effort and never silently substitute another profile or model.
...
- Put exact deterministic commands in `required_checks` when creating or batch-applying a task. Assign the narrowest checks that prove that slice, including strict lint, generated-contract, or metadata checks in the first task that can introduce those failures. Workers use focused checks while iterating, then invoke each exact command with `worker check` and attach the resulting receipt to `worker submit`. The primary reruns each required check once with `submission check` at the exact submitted commit before acceptance. Worker receipts never satisfy reviewer obligations. Reserve a complete cross-project suite for the final integrated audit unless a task's boundary itself requires it.
