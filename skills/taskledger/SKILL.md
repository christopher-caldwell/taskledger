...
Use the installed CLI as the durable authority. The stronger primary model plans and verifies; Taskledger stores the approved plan and execution history.
...
The default store is `<repository>/.taskledger/`. The CLI can continue an accessible v0.1 shared-home project. If discovery reports that legacy storage is unavailable, rerun the same command with the user's existing `TASKLEDGER_HOME=~/.taskledger`, requesting narrowly scoped filesystem approval when required. Say: “Taskledger is already set up here, but I need permission to load its existing data.” If access is denied, stop before the first unapproved gate. Never continue with chat-only gates, initialize a duplicate, or invent a second home to bypass project state.
...
2. **Execution:** exact branch, HEAD or unborn state, cleanliness, `.taskledger/` ignore status, exact commit/init/materialization actions, validation result, safe worktree prerequisites, and the first assignment wave. Inventory ignored environment files, local databases/services, generated artifacts, certificates, and other non-Git inputs required by worker checks. Record a safe setup command or canonical read-only path in the affected task scope; never copy credentials into Git or prompts. Ask for explicit permission to perform the named mutations and launch that wave.
...
Delegated execution requires two project-configurable custom agents:

- `taskledger_worker_routine` for fully specified work with a frozen approach, bounded ownership, and deterministic checks;
- `taskledger_worker_complex` for work that still requires approach selection, cross-boundary reconciliation, or high-consequence judgment.

Project agents live at `.codex/agents/taskledger-worker-routine.toml` and `.codex/agents/taskledger-worker-complex.toml`; personal agents may use the equivalent paths under `~/.codex/agents/`. Reusable templates are [taskledger-worker-routine.toml](assets/taskledger-worker-routine.toml) and [taskledger-worker-complex.toml](assets/taskledger-worker-complex.toml). The consuming repository owns each agent's model, reasoning effort, and any repository-specific worker instructions.

The stronger primary orchestrator must classify every assignment before creation and pass `worker_profile` as `routine` or `complex`. **Prefer `routine`.** Size, file count, and the word “migration” do not make a task complex. A large mechanical rollout may remain routine when all of these are true:
...
- A cross-file contract or port rollout is `routine` when the orchestrator has frozen the exact types, ownership, migration pattern, and executable checks.
...
- Confirm the selected named agent is available before creating an assignment intended for delegation. Never substitute the other profile, a generic subagent, or a catch-all default silently.
- After Approval 2, create the assignment with its selected profile and spawn only the matching named agent. When calling `spawn_agent`, pass `fork_turns: "none"`; never omit it or use `"all"`. Taskledger's persisted worker snapshot is the context boundary, so workers must not inherit the orchestrator conversation. Distinct assignments may run together only under the approved parallel wave; never give one assignment to two workers concurrently.
- Make the context-free spawn message self-contained: give the worker the assignment ID, task ID, profile, worktree path, worker token path, and context hash, and tell it to load the persisted snapshot with `worker context`. Never expose the raw worker token.
...
- The primary orchestrator loads `submission review-context`, independently inspects and tests the exact submitted commit, and decides every criterion. Worker evidence remains a claim.
- Put exact deterministic commands in `required_checks` when creating or batch-applying a task. Assign the narrowest checks that prove that slice, including strict lint, generated-contract, or metadata checks in the first task that can introduce those failures. Workers use focused checks while iterating, then run the complete required set after their final change and submit matching command/exit-code evidence. The primary reruns the required checks independently once per submitted state. Reserve a complete cross-project suite for the final integrated audit unless a task's boundary itself requires it.
...
Do not use `agents.default_subagent_model` for either profile. The primary orchestrator remains responsible for decomposition, routing, independent review, integration, and completion.
...
- Run commands from the managed repository so Taskledger finds its repository-local store. A legacy shared ledger requires the user's explicit `TASKLEDGER_HOME` override.
...
Create only the Approval 2 assignment wave. Implement in assignment worktrees, submit through `worker submit`, independently inspect and test the exact commit, verify every criterion, and integrate. After the last implementation task, the primary orchestrator—not a generic “final hardening” worker—audits the integrated canonical diff across task seams, runs the complete cross-project gates once, and verifies ready requirements. If that audit finds a defect, create the smallest correction task and route it by actual complexity; do not delegate a duplicate full audit and then review the reviewer. Complete the project only when no work, reviews, blockers, invalid plan, or unresolved operations remain. Successful `project complete` automatically removes clean, finalized Taskledger assignment worktrees while retaining every assignment branch and commit. Report any skipped dirty, locked, mismatched, or otherwise unsafe worktree; after the user preserves or cleans it, retry with `project cleanup`.
...
- `LEDGER_STORAGE_UNAVAILABLE` means Taskledger data cannot be accessed; it does not mean project recovery is required. For a legacy shared store, request scoped access and retry the same command. If access remains unavailable, stop rather than substituting a chat-only plan.
...
Treat this local v0.5.2 implementation as experimental. Its happy-path acceptance suite passes, but its full failure-injection and conformance suite is not complete. For high-risk or irreplaceable work, surface that limitation before relying on interrupted-operation guarantees.
