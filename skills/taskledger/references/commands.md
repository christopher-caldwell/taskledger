...
- `controller report`: `{"run_id":"uuid","include_provider_detail":true}`. Produces deterministic structured reporting from Taskledger data and optional Codex persisted-history enrichment. It never starts a model turn and still succeeds with provider detail unavailable.
